...
const MarkerStyleManager = ({ onChange, t: tProp }) => {
  ...
  const addRule = () => {
    const next = [...elevationRules, { id: Date.now(), minElev: '', maxElev: '', color: '#a855f7', label: t('panels.newRange') }];
    setElevationRules(next);
    notify({ elevationRules: elevRulesEnabled ? next : [] });
  };
  ...
  return (
    <div style={{ background: 'rgba(15,32,64,0.92)', border: '1px solid rgba(255,255,255,0.10)', borderRadius: '12px', padding: '12px', color: '#cbd5e1', fontSize: '10px', lineHeight: 1.5 }}>
      <div style={{ fontWeight: 800, marginBottom: '8px', color: '#e0eaff', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
        {t('panels.markerStylesTitle')}
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: '4px', marginBottom: '10px' }}>
        {tabBtn('color', `🎨 ${t('panels.colorTab')}`)}
        {tabBtn('icons', `📌 ${t('panels.iconsTab')}`)}
        {tabBtn('legend', `📋 ${t('panels.legendTab')}`)}
      </div>
      ...
      {activeTab === 'color' && (
        <div style={{ display: 'grid', gap: '8px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
            ...
            <span style={{ fontSize: '9px', color: '#e2e8f0' }}>{t('panels.colorByElevation')}</span>
          </label>

          {/* External size scale */}
          <div>
            <div style={{ fontSize: '9px', color: '#94a3b8', marginBottom: '3px' }}>
              {t('panels.pointSizeScale')}: <strong style={{ color: '#e0eaff' }}>{pointSizeScale.toFixed(1)}×</strong>
            </div>
            <input
          ...
          {elevRulesEnabled && (
            <div style={{ display: 'grid', gap: '5px' }}>
              <div style={{ fontSize: '8px', color: '#93c5fd', fontWeight: 600 }}>{t('panels.elevationRanges')}</div>
              {elevationRules.map((rule) => (
                <div key={rule.id} style={{ display: 'grid', gridTemplateColumns: '22px 1fr 1fr 1.5fr 22px', gap: '4px', alignItems: 'center', background: 'rgba(15,23,42,0.5)', padding: '5px', borderRadius: '5px' }}>
                  <input
                    ...
                    title={t('panels.pickColor')}
                  />
                  <input type="number" placeholder={t('panels.min')} value={rule.minElev} onChange={(e) => updateRule(rule.id, 'minElev', e.target.value)} style={inputStyle} />
                  <input type="number" placeholder={t('panels.max')} value={rule.maxElev} onChange={(e) => updateRule(rule.id, 'maxElev', e.target.value)} style={inputStyle} />
                  <input type="text" placeholder={t('panels.label')} value={rule.label} onChange={(e) => updateRule(rule.id, 'label', e.target.value)} style={inputStyle} />
      ...
      {activeTab === 'icons' && (
        <div style={{ display: 'grid', gap: '7px' }}>
          <div style={{ fontSize: '9px', color: '#94a3b8' }}>{t('panels.uploadIconsHelp')}</div>
          {SOURCE_TYPES.map((src) => (
            <div key={src.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(15,23,42,0.5)', padding: '6px 8px', borderRadius: '6px' }}>
              ...
              <button type="button" onClick={() => fileInputRefs.current[src.id]?.click()} style={{ border: '1px solid rgba(148,163,184,0.45)', background: 'rgba(37,99,235,0.2)', color: '#93c5fd', borderRadius: '4px', fontSize: '8px', padding: '3px 7px', cursor: 'pointer' }}>{t('panels.upload')}</button>
              {customIcons[src.id] && (
          ...
          <div style={{ fontSize: '8px', color: '#64748b' }}>{t('panels.iconsRenderSize')}</div>
        </div>
      )}
      ...
      {activeTab === 'legend' && (
        <div style={{ display: 'grid', gap: '8px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
            ...
            <span style={{ fontSize: '9px', color: '#e2e8f0' }}>{t('panels.showLegendOverlay')}</span>
          </label>

          {/* Preview */}
          <div style={{ background: 'rgba(15,23,42,0.7)', borderRadius: '6px', padding: '8px', border: '1px solid rgba(148,163,184,0.2)' }}>
            <div style={{ fontSize: '9px', color: '#93c5fd', fontWeight: 700, marginBottom: '6px' }}>{t('panels.legendPreview')}</div>

            {Object.entries(customIcons).length > 0 && (
              <div style={{ marginBottom: '6px' }}>
                <div style={{ fontSize: '8px', color: '#94a3b8', marginBottom: '3px', textTransform: 'uppercase', letterSpacing: '0.03em' }}>{t('panels.customIcons')}</div>
            ...
            {elevRulesEnabled && elevationRules.length > 0 && (
              <div>
                <div style={{ fontSize: '8px', color: '#94a3b8', marginBottom: '3px', textTransform: 'uppercase', letterSpacing: '0.03em' }}>{t('panels.elevationColors')}</div>
                {elevationRules.map((rule) => (
