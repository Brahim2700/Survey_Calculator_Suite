...
const buildCadSummaryHtml = (report) => {
  ...
  const metricRows = [
    ...
    ["Processing Mode", inspection?.processingMode],
...
const buildCadInspectionSummary = (file, rows, status, payload = null, strictExistingPointsOnly = true, projectionDiagnostics = null) => {
  ...
  return {
    ...
    processingMode: payload?.inspection?.processingMode || "full",
...
const CAD_VALIDATION_SEVERITY_WEIGHT = {
  error: 3,
  warning: 2,
  info: 1,
};

const buildCadValidationIssueRows = (inspection) => {
  if (!inspection) return [];

  const rows = [];
  let seq = 1;
  const addIssue = ({ severity = "info", category = "general", code = "", title = "", message = "", detail = "" }) => {
    const normalizedSeverity = CAD_VALIDATION_SEVERITY_WEIGHT[severity] ? severity : "info";
    rows.push({
      id: `${code || category || "cad"}-${seq++}`,
  ...
  const notifications = Array.isArray(inspection.validation?.notifications) ? inspection.validation.notifications : [];
  notifications.forEach((item) => {
    addIssue({
      severity: item?.severity || "info",
      category: "validation",
      code: item?.code || "cad-validation",
      title: item?.title || "Validation notice",
      message: item?.message || "",
      detail: "Auto-generated by CAD validation subsystem",
    });
  });

  if (inspection.processingMode === "recovery") {
    addIssue({
      severity: "warning",
      category: "processing",
      code: "cad-recovery-mode",
      title: "Recovery mode import",
      message: "This file was imported in recovery mode. Some advanced entities may be simplified or omitted.",
      detail: "Review diagnostics and re-run full import when possible.",
    });
  }

  if (inspection.processingMode === "preview") {
    addIssue({
      severity: "info",
      category: "processing",
      code: "cad-preview-mode",
      title: "Preview mode import",
      message: "Preview mode was used to reduce load time for this file.",
      detail: "Run full mode if you need complete entity coverage.",
    });
  }

  if (inspection.preflightModeConfidence === "low") {
    addIssue({
      severity: "warning",
      category: "preflight",
      code: "cad-preflight-low-confidence",
  ...
  const unresolvedXrefs = Array.isArray(inspection.diagnostics?.references?.unresolvedXrefs)
    ? inspection.diagnostics.references.unresolvedXrefs
    : [];
  unresolvedXrefs.forEach((xref) => {
    addIssue({
      severity: "warning",
      category: "reference",
      code: "cad-unresolved-xref",
      title: "Unresolved XREF",
      message: `Missing external reference: ${xref?.name || "(unnamed)"}`,
      detail: `Layer: ${xref?.layer || "unknown"} | Depth: ${xref?.depth ?? 0}`,
    });
  });

  const unresolvedBlocks = Array.isArray(inspection.diagnostics?.references?.unresolvedBlockRefs)
    ? inspection.diagnostics.references.unresolvedBlockRefs
    : [];
  unresolvedBlocks.forEach((block) => {
    addIssue({
      severity: "warning",
      category: "reference",
      code: "cad-missing-block",
      title: "Missing block definition",
      message: `Missing block: ${block?.name || "(unnamed)"}`,
      detail: `Layer: ${block?.layer || "unknown"} | Depth: ${block?.depth ?? 0}`,
    });
  });

  const cyclicBlocks = Array.isArray(inspection.diagnostics?.references?.cyclicBlockRefs)
    ? inspection.diagnostics.references.cyclicBlockRefs
    : [];
  cyclicBlocks.forEach((cycle) => {
    addIssue({
      severity: "warning",
      category: "reference",
      code: "cad-cyclic-block",
      title: "Cyclic block reference",
      message: `Cycle detected: ${cycle?.name || "block"}`,
      detail: Array.isArray(cycle?.chain) ? cycle.chain.join(" -> ") : "Cyclic chain unavailable",
    });
  });

  if (inspection.layerSummary?.renamedLayers > 0) {
    addIssue({
      severity: "info",
      category: "layers",
      code: "cad-layers-normalized",
      title: "Layer normalization applied",
      message: `${inspection.layerSummary.renamedLayers} layer name(s) normalized and standardized.`,
      detail: `Standardized layers: ${inspection.layerSummary.totalStandardizedLayers ?? inspection.layerSummary.layers?.length ?? 0}`,
    });
  }

  const repairEntries = Object.entries(inspection.repairs || {}).filter(([, value]) => Number(value) > 0);
  repairEntries.forEach(([key, value]) => {
    addIssue({
      severity: "info",
      category: "repair",
      code: `cad-repair-${key}`,
      title: `Auto repair: ${key}`,
      message: `${value} automatic repair operation(s) applied.`,
...
const CoordinateConverter = ({ uiLanguage = 'en', t: tFromProps }) => {
  ...
  const filteredCadValidationRows = useMemo(() => {
    ...
    const rows = severityFilter === "all"
      ? cadValidationRows
      : cadValidationRows.filter((row) => String(row.severity).toLowerCase() === severityFilter);

    const sorted = [...rows].sort((a, b) => {
      if (cadValidationSortBy === "severity") {
        const diff = (CAD_VALIDATION_SEVERITY_WEIGHT[b.severity] || 0) - (CAD_VALIDATION_SEVERITY_WEIGHT[a.severity] || 0);
        if (diff !== 0) return diff;
      }
      const left = String(a[cadValidationSortBy] || "").toLowerCase();
      const right = String(b[cadValidationSortBy] || "").toLowerCase();
      if (left < right) return -1;
      if (left > right) return 1;
      return 0;
    });

    return cadValidationSortDir === "asc" ? sorted : sorted.reverse();
  }, [cadValidationRows, cadValidationSeverityFilter, cadValidationSortBy, cadValidationSortDir]);
  ...
  const runCadPurgeApplyForSelectedFile = useCallback(async () => {
    ...
    try {
      ...
      if (diagnostics) {
        ...
        console.log('has utf8 bom:', diagnostics.hasUtf8Bom);
        ...
        console.log('group code pair count:', diagnostics.groupCodePairCount);
        console.log('validation errors:', diagnostics.validationErrors);
        console.log('output head utf8 (first 200 bytes):', diagnostics.outputArtifact?.headUtf8);
        console.log('output tail utf8 (last 200 bytes):', diagnostics.outputArtifact?.tailUtf8);
        console.log('output head hex (first 200 bytes):', diagnostics.outputArtifact?.headHex);
        console.log('output tail hex (last 200 bytes):', diagnostics.outputArtifact?.tailHex);
  ...
  return (
    <div style={{ width: "100%", maxWidth: "1100px", background: "var(--c-surface, #fff)", borderRadius: "var(--r-lg, 14px)", padding: "1.5rem", boxShadow: "var(--shadow-xl, 0 20px 60px rgba(0,0,0,.22))", display: "flex", flexDirection: "column", border: "1px solid rgba(255,255,255,.08)" }}>
      ...
      <>
        ...
        <div style={{ border: "1px solid #cbd5e1", borderRadius: "8px", background: "#ffffff", padding: "0.75rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.6rem", flexWrap: "wrap" }}>
            <div style={{ fontWeight: 700, color: "#0f172a", fontSize: "0.76rem", textTransform: "uppercase", letterSpacing: "0.06em" }}>CAD Validation Panel</div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.45rem", flexWrap: "wrap" }}>
              <select
                value={cadValidationSeverityFilter}
                onChange={(e) => setCadValidationSeverityFilter(e.target.value)}
                style={{ borderRadius: "6px", border: "1px solid #cbd5e1", padding: "0.25rem 0.45rem", fontSize: "0.78rem" }}
              >
              ...
              <select
                value={cadValidationSortBy}
                onChange={(e) => setCadValidationSortBy(e.target.value)}
                style={{ borderRadius: "6px", border: "1px solid #cbd5e1", padding: "0.25rem 0.45rem", fontSize: "0.78rem" }}
              >
              ...
              <button
                onClick={() => setCadValidationSortDir((dir) => (dir === "asc" ? "desc" : "asc"))}
                style={{ padding: "0.25rem 0.55rem", borderRadius: "6px", border: "1px solid #cbd5e1", background: "#f8fafc", color: "#334155", fontWeight: 600, fontSize: "0.78rem", cursor: "pointer" }}
              >
                {cadValidationSortDir === "asc" ? "Ascending" : "Descending"}
              </button>
            </div>
          </div>

          {!cadInspection && (
            <div style={{ marginTop: "0.55rem", fontSize: "0.84rem", color: "#64748b" }}>
              Import a DXF/DWG file first to populate anomaly and consistency checks.
            </div>
          )}

          {cadInspection && (
            <>
              <div style={{ marginTop: "0.55rem", fontSize: "0.8rem", color: "#475569" }}>
                Showing {filteredCadValidationRows.length} issue(s) from validation, references, layer normalization, and auto-repair subsystems.
              </div>
              <div style={{ marginTop: "0.55rem", maxHeight: "240px", overflow: "auto", border: "1px solid #e2e8f0", borderRadius: "6px" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem" }}>
                  <thead style={{ background: "#f8fafc", position: "sticky", top: 0, zIndex: 1 }}>
                    <tr>
                      <th style={{ textAlign: "left", padding: "0.45rem", borderBottom: "1px solid #e2e8f0" }}>Severity</th>
                      <th style={{ textAlign: "left", padding: "0.45rem", borderBottom: "1px solid #e2e8f0" }}>Category</th>
                      <th style={{ textAlign: "left", padding: "0.45rem", borderBottom: "1px solid #e2e8f0" }}>Issue</th>
                      <th style={{ textAlign: "left", padding: "0.45rem", borderBottom: "1px solid #e2e8f0" }}>Detail</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredCadValidationRows.length === 0 && (
                      <tr>
                        <td colSpan={4} style={{ padding: "0.65rem", color: "#64748b", textAlign: "center" }}>No issues for the current filter.</td>
                      </tr>
                    )}
                    {filteredCadValidationRows.map((issue) => {
                      const tone = issue.severity === "error"
                        ...
                        : issue.severity === "warning"
                          ...
                          : { bg: "#f8fafc", fg: "#1e293b" };
                      return (
                        <tr key={issue.id} style={{ background: tone.bg }}>
                          <td style={{ padding: "0.4rem", borderBottom: "1px solid #e2e8f0", color: tone.fg, fontWeight: 700, textTransform: "uppercase" }}>{issue.severity}</td>
                          <td style={{ padding: "0.4rem", borderBottom: "1px solid #e2e8f0", color: "#334155", textTransform: "capitalize" }}>{issue.category}</td>
                          <td style={{ padding: "0.4rem", borderBottom: "1px solid #e2e8f0", color: "#0f172a" }}>
                            <div style={{ fontWeight: 600 }}>{issue.title}</div>
                            <div style={{ color: "#475569" }}>{issue.message}</div>
                          </td>
                          <td style={{ padding: "0.4rem", borderBottom: "1px solid #e2e8f0", color: "#475569", whiteSpace: "pre-wrap" }}>{issue.detail || "-"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
